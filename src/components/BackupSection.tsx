...
export function BackupSection({ onExport, onImport }: BackupSectionProps) {
  ...
  const doImport = async (file: File | undefined) => {
    if (!file) return
    if (!window.confirm('Импортировать копию? Текущие галочки, результаты проверок, теория, журнал ошибок, заметки и правки плана будут заменены.')) {
    ...
    setMessage(
      onImport(raw)
        ? 'Готово: галочки, результаты проверок, теория, журнал ошибок, заметки и правки плана применены.'
    ...
    <section className="page-section" aria-labelledby="backup-title">
      <details className="section-fold">
        ...
        <p className="section-lead section-fold__lead">
          Прогресс хранится в localStorage конкретного браузера. Экспорт скачивает всё (галочки, результаты проверок, теорию, журнал ошибок, второй круг, заметки,
          правки плана) одним файлом JSON — импорт на другом устройстве заменяет им текущее состояние.
        </p>
